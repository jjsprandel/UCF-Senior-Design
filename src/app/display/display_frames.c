...
void display_test_1(void *pvParameters)
{
    ...
    const spi_bus_config_t buscfg = GC9A01_PANEL_BUS_SPI_CONFIG(EXAMPLE_PIN_NUM_SCLK, EXAMPLE_PIN_NUM_MOSI,
                                                                EXAMPLE_LCD_H_RES * 80 * TEST_LCD_BIT_PER_PIXEL / 8);
    spi_bus_initialize(EXAMPLE_LCD_HOST, &buscfg, SPI_DMA_CH_AUTO);
    ...
    esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)EXAMPLE_LCD_HOST, &io_config, &io_handle);
...
void display_test(void *pvParameters)
{
    ...
    void *buf1 = spi_bus_dma_memory_alloc(EXAMPLE_LCD_HOST, draw_buffer_sz, 0);
    assert(buf1);
    void *buf2 = spi_bus_dma_memory_alloc(EXAMPLE_LCD_HOST, draw_buffer_sz, 0);
    assert(buf2);
    ...
    ESP_ERROR_CHECK(esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)EXAMPLE_LCD_HOST, &tp_io_config, &tp_io_handle));
    ...
    ESP_LOGI(DISPLAY_TAG, "Initialize touch controller STMPE610");
